...
export function emitNewProjectileLaunchEvents(io, room, entry) {
  ...
  const recipients = [...recipientSocketIds(room, entry)];
...
/** projectile 最终碰撞/伤害由服务端决定，消失时即时发 despawn。 */
...
function stripPrivateAggregateFields(basic) {
  const {
    resourcesByUser: _resourcesByUser,
    skillsByUser: _skillsByUser,
    units: _units,
    ...shared
  } = basic ?? {};
  return shared;
...
function broadcastSnapshots(io, room, entry, options = {}) {
  emitPersonalized(io, room, entry, 'pvp:authority:snapshot', options);
}

function deferBroadcast(io, room, entry, excludeUserId = null) {
  // ACK 先排入当前玩家连接，其他玩家的全量状态包装移到下一事件循环，降低操作手感延迟。
  setImmediate(() => {
    if (!roomManager.getRoom(room.id) || !authorityBattles.has(Number(room.id))) return;
    broadcastSnapshots(io, room, entry, { excludeUserId, includeProjectiles: false });
  });
}
...
function ensureAuthorityBattle(roomId, io, cardDb) {
  ...
  entry.timer = setInterval(() => {
    ...
    while (entry.accumulator >= STEP_SECONDS) {
      const activeProjectiles = entry.battle.engine?.projectiles ?? [];
      const previousProjectiles = activeProjectiles.length
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('pvp:authority:spectate', (payload = {}, ack) => {
      try {
        ...
        const snapshot = buildSpectatorSnapshot(entry, socket.user.id, undefined, null, { includeProjectiles: true });
        socket.emit('pvp:authority:snapshot', snapshot);
        ackOk(ack, {
          userId: Number(socket.user.id),
          team: 'blue',
    ...
    socket.on('pvp:authority:set-loadout', (payload = {}, ack) => {
      try {
        ...
        deferBroadcast(io, room, entry, socket.user.id);
      } catch (error) {
        ackError(ack, error);
      }
    });
